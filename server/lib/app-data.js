...
function setAppData(request, data, path) {
  ...
  if (path) {
    validateAppData(existingAppData, path)
  }

  console.log(
    Color.FgCyan,
    "session data before update " + JSON.stringify(existingAppData, null, 4)
  ) //TODO Remove this
  ...
  console.log(
    Color.FgGreen,
    "session data after update " + JSON.stringify(mergedAppData, null, 4)
  ) //TODO Remove this
...
function getAppFlow(appData) {
  ...
  if (appData?.permitType === "other") {
    appFlow.push("cannot-use-service")
  }
  if (appData?.permitType && appData?.permitType !== "other") {
    ...
    if (
      appData?.isAgent === false ||
      (appData?.isAgent === true && appData?.agent?.address)
    ) {
